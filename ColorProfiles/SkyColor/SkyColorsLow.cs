namespace FancyLighting.ColorProfiles.SkyColor;

public sealed class SkyColorsLow : LoadableColorProfileBase, ISimpleColorProfile
{
    private readonly SkyColorProfile _profile;

    private SkyColorsLow()
    {
        _profile = new(InterpolationMode.Cubic);

        var noonTime = 12.0;
        var sunriseTime = noonTime - (6.0 + (50.0 / 60.0));
        var sunsetTime = noonTime + (6.0 + (50.0 / 60.0));

        var nightColor = new Vector3(0.04f, 0.04f, 0.055f);
        var nightColor3 = new Vector3(0.05f, 0.05f, 0.09f);
        var nightColor2 = new Vector3(0.08f, 0.07f, 0.18f);
        var nightColor1 = new Vector3(0.37f, 0.28f, 0.28f);
        var sunriseSunsetColor = new Vector3(0.76f, 0.55f, 0.33f);
        var dayColor1 = new Vector3(0.84f, 0.72f, 0.47f);
        var dayColor2 = new Vector3(0.88f, 0.81f, 0.7f);
        var dayColor3 = new Vector3(0.68f, 0.81f, 0.92f);
        var dayColor = new Vector3(0.58f, 0.81f, 1f);

        (double hour, Vector3 color)[] colors =
        [
            (0.00, nightColor),
            (sunriseTime - 2.001, nightColor),
            (sunriseTime - 2.0, nightColor),
            (sunriseTime - 1.25, nightColor3),
            (sunriseTime - 0.9, nightColor2),
            (sunriseTime - 0.4, nightColor1),
            (sunriseTime, sunriseSunsetColor),
            (sunriseTime + 0.75, dayColor1),
            (sunriseTime + 1.5, dayColor2),
            (sunriseTime + 2.25, dayColor3),
            (sunriseTime + 3.0, dayColor),
            (sunriseTime + 3.001, dayColor),
            (noonTime, dayColor),
            (sunsetTime - 3.001, dayColor),
            (sunsetTime - 3.0, dayColor),
            (sunsetTime - 2.25, dayColor3),
            (sunsetTime - 1.5, dayColor2),
            (sunsetTime - 0.75, dayColor1),
            (sunsetTime, sunriseSunsetColor),
            (sunsetTime + 0.4, nightColor1),
            (sunsetTime + 0.9, nightColor2),
            (sunsetTime + 1.25, nightColor3),
            (sunsetTime + 2.0, nightColor),
            (sunsetTime + 2.001, nightColor),
        ];

        foreach (var (hour, color) in colors)
        {
            _profile.AddColor(hour, color);
        }
    }

    public Vector3 GetColor(double hour) => _profile.GetColor(hour);
}
