...
public abstract class FancyLightingEngineVecDecay : FancyLightingEngineBase
{
    private Vec3[] _lightAirDecay;
    protected Vec3[] _lightSolidDecay;
    private Vec3[] _lightWaterDecay;
    private Vec3[] _lightHoneyDecay;
    private Vec3[] _lightNonSolidDecay;

    protected Vec3[][] _lightMasks;

    protected void InitializeDecayArrays()
    {
        _lightAirDecay = new Vec3[DistanceTicks + 2];
        _lightSolidDecay = new Vec3[DistanceTicks + 2];
        _lightWaterDecay = new Vec3[DistanceTicks + 2];
        _lightHoneyDecay = new Vec3[DistanceTicks + 2];
        _lightNonSolidDecay = new Vec3[DistanceTicks + 2];
    ...
    protected void UpdateDecays(LightMap lightMap)
    {
        ...
        var lightWaterDecayBaseline = Vector3.Min(
            lightMap.LightDecayThroughWater,
            new(MaxDecayMult)
        );
        var lightHoneyDecayBaseline = Vector3.Min(
            lightMap.LightDecayThroughHoney,
            new(MaxDecayMult)
        );
        ...
        var logSlowestDecay = MathF.Log(
            Math.Max(
                ...
                Math.Max(
                    Math.Max(
                        lightWaterDecayBaseline.X,
                        Math.Max(lightWaterDecayBaseline.Y, lightWaterDecayBaseline.Z)
                    ),
                    Math.Max(
                        lightHoneyDecayBaseline.X,
                        Math.Max(lightHoneyDecayBaseline.Y, lightHoneyDecayBaseline.Z)
                    )
                )
            )
        );
    ...
    private static void UpdateDecay(Vec3[] decay, float baseline)
    {
        if (new Vec3(baseline) == decay[DistanceTicks])
        {
            return;
        }
        ...
        {
            decay[i] = new(MathF.Exp(ExponentMult * i * logBaseline));
        }

        // Last element is for diagonal decay (used in GI)
        decay[^1] = new(MathF.Exp(MathF.Sqrt(2) * logBaseline));
    }

    private static void UpdateDecay(Vec3[] decay, Vector3 baseline)
    {
        if (new Vec3(baseline.X, baseline.Y, baseline.Z) == decay[DistanceTicks])
        {
            return;
        }

        var logBaselineX = MathF.Log(baseline.X);
        var logBaselineY = MathF.Log(baseline.Y);
        var logBaselineZ = MathF.Log(baseline.Z);
        const float ExponentMult = 1f / DistanceTicks;
        for (var i = 0; i < decay.Length; ++i)
        {
            var exponent = ExponentMult * i;
            decay[i] = new(
                MathF.Exp(exponent * logBaselineX),
                MathF.Exp(exponent * logBaselineY),
                MathF.Exp(exponent * logBaselineZ)
            );
        }

        // Last element is for diagonal decay (used in GI)
        decay[^1] = new(
            MathF.Exp(MathF.Sqrt(2f) * logBaselineX),
            MathF.Exp(MathF.Sqrt(2f) * logBaselineY),
            MathF.Exp(MathF.Sqrt(2f) * logBaselineZ)
        );
    }
